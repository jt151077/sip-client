...
function sipRegister() {
    ...
    try {
        ...
        oSipStack = new SIPml.Stack({
                ...
                //websocket_proxy_url: 'wss://communications-ims-1019610563.eu-west-1.elb.amazonaws.com:443',
                websocket_proxy_url: 'wss://ec2-52-209-113-185.eu-west-1.compute.amazonaws.com:443',
                //websocket_proxy_url: (window.localStorage ? window.localStorage.getItem('org.doubango.expert.websocket_server_url') : null),
                //outbound_proxy_url: 'udp://communications-ims-1019610563.eu-west-1.elb.amazonaws.com:5060',
                outbound_proxy_url: 'udp://ec2-52-209-113-185.eu-west-1.compute.amazonaws.com:5060',
...
function sipSendMessage() {
    ...
    if(messageNameInput.value !== '' && messageInput.value !== '') {
        messageArea.append(moment().format('YYYY-MM-DD HH:mm:ss')+' (me -> '+messageNameInput.value+'): '+ messageInput.value + '\n');
        oSipSessionMessage.send(messageNameInput.value, messageInput.value, 'text/plain;charset=utf-8');
        messageInput.value = '';
        messageArea.scrollTop = messageArea.scrollHeight;
    }
    else {
        alert('Please provide a destination name and a text message !!!');
    }
...
function uiOnConnectionEvent(b_connected, b_connecting) { // should be enum: connecting, connected, terminating, terminated
    ...
    btnSendMessage.disabled = !(b_connected && tsk_utils_have_webrtc() && tsk_utils_have_stream());
...
function onSipEventStack(e /*SIPml.Stack.Event*/) {
        ...
        case 'stopping': case 'stopped': case 'failed_to_start': case 'failed_to_stop':
    {
        ...
        case 'i_new_message':
        {
            ...
            messageArea.append(moment().format('YYYY-MM-DD HH:mm:ss') + ' ('+e.newSession.getRemoteFriendlyName()+'): '+ e.getContentString() + '\n');
            messageArea.scrollTop = messageArea.scrollHeight;
            break;
        }
...
function messageListener(e) {
    console.log('---> messageListener: session event = ' + e.type);
}
...
function onSipEventSession(e /* SIPml.Session.Event */) {
        ...
        case 'connecting': case 'connected':
    {
        ...
        oSipSessionMessage = oSipStack.newSession('message', {
            events_listener: { events: '*', listener: messageListener } // optional: '*' means all events
        });
